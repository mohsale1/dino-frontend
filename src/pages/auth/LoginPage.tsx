...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // System User Bypass - Development/Demo Mode
      if (tabValue === 1 && formData.email === 'admin@dinos.com' && formData.password === 'Admin@123') {
        // Simulate successful system user login
        setLoading(false);
        // Navigate directly to system admin dashboard
      ...
      if (user?.role) {
        // System User Login (Dinos role)
        if (tabValue === 1) {
          if (!isDinos(user.role)) {
            setError('Invalid credentials for system user login');
            setLoading(false);
            return;
          }
          ...
          // Business Login
          if (isDinos(user.role)) {
            setError('System users should use the System User Login tab');
            setLoading(false);
            return;
          }
